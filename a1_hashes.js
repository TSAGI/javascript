var violations = [ {money_owed: 50.0, violation_category: "Garbage and Refuse", date: "2012-11-01 00:00:00", inspection_id: "232528"},
...
// CREATE AN ARRAY CONTAINING VIOLATION CATEGORIES ONLY
var a = violations.map(v => v.violation_category);
// REMOVE DUPLICATES FROM THE ARRAY
var unique = Array.from(new Set(a));

// CREATE AN EMPTY ARRAY REPRESENTING VIOLATION CATEGORIES AND MONEY OWED 
var v_m_owed = []
// POPULATE THE ARRAY: EACH ELEMENT WILL HAVE "VIOLATION_CATEGORY" AS A KEY,
//  AND THE TOTAL NUMBER OF VIOLATIONS AS IT'S VALUE. EACH ELEMENT WILL ALSO HAVE A 
// SECOND KEY CALLED "MONEY_OWED", AND THE TOTAL MONEY OWED AS IT'S VALUE.
for(index in unique) {
    var total = 0
    var count = 0  
    
    violations.forEach(function(obj){
        var owed = obj.money_owed  
        
        if (obj.violation_category == unique[index])
        { 
            total += owed 
            count += 1
        }
    });
    v_m_owed.push({[unique[index]]: count, money_owed: total})
}

// DISPLAY TO CONSOLE
for(element in v_m_owed) { 
    var obj = v_m_owed[element]
    console.log(
        "There were " + Object.values(obj)[0] + " violations in the " 
        + Object.keys(obj)[0] + " category. The total money owed is $"
        + Object.values(obj)[1] + ".00"
    )
};
    
// EXPERIMENTED WITH MAP
// var b = violations.map(v => ({
//     violation_category: v.violation_category,
//     money_owed: v.money_owed
// }));
